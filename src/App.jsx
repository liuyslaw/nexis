  ...
  const runExtraction = async () => {
    ...
    for (let fi=0; fi<eligibleFiles.length; fi++) {
      ...
      try {
        ...
        if (result.success) {
          const assessment = assessConfidence(result.data);
          ...
          setProgress(p=>p.map(x=>x.id===f.id?{...x,status:'done',conf:assessment.overall,escalated:!!result.escalated,escalation_failed:!!enriched._escalation_failed}:x));
        } else {
