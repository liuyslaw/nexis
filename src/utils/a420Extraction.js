...
SECURITY EXTRACTION — critical field, often in middle pages:
Look for section titled "Securities and Documentation", "Security", or "12. Securities".
It contains: Facilities Agreement (principal sum), Properties With Title — Legal Charge (property descriptions), Joint and Several Guarantee (guarantor names and amounts).
Format: "Facilities Agreement - upstamp to secure RM[X]\nProperties with title - Legal Charge:\n- [property description]\nJoint and several guarantee of RM[X] by:\n- [Name 1]\n- [Name 2]"
Set confidence to 0.9 if found. If not visible in extracted text, set to null and add review note.
...
async function runExtraction(messages, model, fileName, pageTexts, declaredCategory) {
  ...
  // Robust JSON extraction — strip preamble text before/after JSON block
...
export async function extractA420Fields(pageTexts, fileName, declaredCategory, declaredDocType) {
  ...
  // 3-section strategy: cover+facility table | security section (middle) | end T&C
  // Security in HLB offer letters is on pages 5-8 — falls in the middle, missed by start+end only
  let combinedText;
  if (fullText.length <= 6000) {
    combinedText = fullText;
  } else {
    const first    = fullText.slice(0, 3000);
    const midStart = Math.floor(fullText.length * 0.35);
    const mid      = fullText.slice(midStart, midStart + 2500);
    const last     = fullText.slice(-1500);
    combinedText = first + '\n\n[...]\n\n' + mid + '\n\n[...]\n\n' + last;
  }
