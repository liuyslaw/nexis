...
Return a JSON ARRAY — one object per facility found in the document:
...
If document has only one facility, return array with one object: [{"facility_category":...}]
...
Signs: titled "Supplemental Letter", "Variation", "Amendment", or contains "upstamp", "revised terms".
- facility_limit_rm = ORIGINAL approved limit, NOT any outstanding balance shown in letter body (e.g. RM51,538.68 is a remaining balance, not the approved limit)
- Add review note: "Amendment letter — verify original approved limit from first offer letter."
...
async function runExtraction(messages, model, fileName, pageTexts, declaredCategory) {
  const response = await fetchWithRetry('/api/ai', {
    ...
    body: JSON.stringify({ messages, temperature: 0.05, max_tokens: 1024, provider: 'anthropic', model })
  });
  ...
  // Robust JSON extraction — handles both array [...] and object {...} responses
  ...
  if (arrStart !== -1 && (objStart === -1 || arrStart < objStart)) {
    ...
    jsonStr = arrEnd > arrStart ? clean.slice(arrStart, arrEnd + 1) : clean;
  } else {
    // Object response — wrap in array
    const objEnd = clean.lastIndexOf('}');
    jsonStr = objEnd > objStart ? '[' + clean.slice(objStart, objEnd + 1) + ']' : clean;
  }
  const parsedArr = JSON.parse(jsonStr);
  // Always work with array internally; return first element for compatibility
  const facilities = Array.isArray(parsedArr) ? parsedArr : [parsedArr];
  const parsed = facilities[0]; // primary facility for runExtraction return
  parsed._all_facilities = facilities; // store all for multi-facility handling
...
export async function extractA420Fields(pageTexts, fileName, declaredCategory, declaredDocType) {
  ...
  try {
    ...
    // Enrich all facilities with metadata
    const allFacilities = parsed._all_facilities || [parsed];
    allFacilities.forEach((f, i) => {
      if (!['L','HP'].includes(f.facility_category)) f.facility_category = declaredCategory || 'L';
      if (!Array.isArray(f.review_notes)) f.review_notes = [];
      if (f.amount_utilised_rm == null) {
        const hasNote = f.review_notes.some(n => n.toLowerCase().includes('utilised'));
        if (!hasNote) f.review_notes.push('Obtain utilised amount from bank confirmation — not in this document');
      }
      f._source_file      = fileName;
      f._extraction_model = HAIKU;
      f._extracted_at     = new Date().toISOString();
      f._page_count       = pageTexts.length;
      f._facility_index   = i;
    });
    return { success: true, data: allFacilities[0], facilities: allFacilities, escalated: false };
  } catch (err) {
    return {
      success: false,
